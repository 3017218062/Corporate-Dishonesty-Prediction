...
class myModel:
    ...
    def dataPreprocessing(self):
        ...
        self.xTrain, self.xPred = None, None
        self.transformed = True
        print("\033[32m  Open train data.\033[0m")
        if os.path.exists("output/notepad/train.csv") and os.path.exists("output/notepad/test.csv"):
            self.xTrain = pd.read_csv("output/notepad/train.csv", low_memory=False)
            self.xPred = pd.read_csv("output/notepad/test.csv", low_memory=False)
    ...
    def featureEngineering(self):
        def dataClearing(data):
            ...
            indices.sort()
            np.save("output/notepad/indices", np.array(indices))
        ...
        if self.transformed:
            # self.xTrain["Label"] = self.yTrain
            # self.xTrain, self.yTrain = dataClearing(self.xTrain)
            ...
            self.xTrain.to_csv("output/notepad/train.csv", index=False)
            self.xPred.to_csv("output/notepad/test.csv", index=False)
        # if self.yTrain.shape[0] != self.xTrain.shape[0]:
        #     self.yTrain = self.yTrain[np.load("output/notepad/indices.npy")]
    ...
    def modelBuilding(self):
        ...
        def xgbc(n_estimators=1000, learning_rate=0.1):
        ...
        def cbc(n_estimators=1000, learning_rate=0.1):
        ...
        for i in range(self.K):
            print("\033[34m  %x) Build selector/model/ensemble\033[0m" % (i + 1))
    ...
    def featureChoosing(self):
        importances = []
        if os.path.exists("output/notepad/importances.npy"):
            importances = np.load("output/notepad/importances.npy")
        else:
            true, pred = [], []
            ...
            for i, (indexTrain, indexTest) in enumerate(kfold.split(self.xTrain, self.yTrain)):
                ...
                sys.stdout.write("\r\033[35m  step %d/%d score: %f" % (i + 1, self.K, roc_auc_score(y2, p)))
                gc.collect()
            score = roc_auc_score(true, pred)
            print("\r\033[35m     %.06f\033[0m" % score)
            importances = np.asarray(importances)
            np.save("output/notepad/importances", importances)
        importances = importances.mean(axis=0)
        # plt.hist(importances, bins=100)
        # plt.xlim((0, 0.01))
        # plt.show()
        # importances = np.argsort(importances)[-238:]
    ...
    def modelFitting(self):
        ...
        for i, (indexTrain, indexTest) in enumerate(kfold.split(self.xTrain, self.yTrain)):
            ...
            pred += list(p)
            sys.stdout.write("\r\033[31m  step %d/%d score: %f" % (i + 1, self.K, roc_auc_score(y2, p)))
            gc.collect()
        score = roc_auc_score(true, pred)
